...
namespace ProBuilder2.Examples
{
	public class CameraControls : MonoBehaviour
	{
		...
		void LateUpdate()
		{
			...
			{
				float rot_x = UnityEngine.Input.GetAxis(INPUT_MOUSE_X);
				float rot_y = -UnityEngine.Input.GetAxis(INPUT_MOUSE_Y);
			...
			if(UnityEngine.Input.GetAxis(INPUT_MOUSE_SCROLLWHEEL) != 0f )
			{
				float delta = UnityEngine.Input.GetAxis(INPUT_MOUSE_SCROLLWHEEL);
